...
const welcomeChannelName = "반갑습니다";
...
client.on('message', (message) => {
  ...
  if(message.content == '!업데이트') {
    return message.reply('투표기능 추가 20/8/26 !업데이트 추가');a
  }
  ...
  if(message.content == 'embedsdsdsdsdseqvs') {
    ...
    let embed = new Discord.RichEmbed()
      ...
      .setAuthor('그린봇', img, 'http://www.naver.com')
      .setThumbnail(img)
      .addBlankField()
  ...
  } else if(message.content.startsWith('!전체공지일반')) {
    if(checkPermission(message)) return
    if(message.member != null) { // 채널에서 공지 쓸 때
      let contents = message.content.slice('!전체공지일반'.length);
      message.member.guild.members.array().forEach(x => {
        if(x.user.bot) return;
...
client.on('message', (message) => { 
	if (message.content.startsWith("그린봇 안녕")) { //메세지가 /안녕 으로 시작된다면
